...
SI.Game.prototype.initializeGame = function () {
	...
	this.tick = function () {
		...
		self.checkEndGame();
		if (!self.finished) self.launchEnemyRocket();
	};
	this.clock = setInterval(this.tick, SI.Sizes.MSPF);
...
SI.Game.prototype.deleteExplodedEnemyShips = function () {
	...
	// Delete each hit once, from right to left, so indices remain valid.
	toDelete = toDelete.filter(function (hit, index, hits) {
		return hits.findIndex(function (other) { return other.row === hit.row && other.col === hit.col; }) === index;
	}).sort(function (a, b) { return b.row - a.row || b.col - a.col; });
	...
	while (i < this.enemies.ships.length) {
		if(this.enemies.ships[i].length == 0) {
			...
			this.enemies.directions.splice(i, 1);
